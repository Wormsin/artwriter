...
def get_stage1_prompt():
    prompt = """
            ...
            События должны быть разделены пустой строкой.
            """
    return prompt
...
def get_stage2_prompt_main(lens_num):
    match lens_num:
        ...
        case 2:
            prompt = f"""
                    [ИНСТРУКЦИЯ]
                    Проанализируй содержимое файлов: db_extension.txt, lens_{lens_num-1}_main.txt.
        ...
        case 3:
            prompt = f"""
                    [ИНСТРУКЦИЯ]
                    Проанализируй содержимое файла lens_{lens_num-1}_main.txt с паттернами.
...
def get_stage2_prompt_blind_spots(lens_num):
    match lens_num:
        case 1:
            prompt = """
                    [ЗАДАЧА]
                    Проанализируй все PDF файлы и db_extension.txt.
                    Найди информацию о том, что НЕ произошло, но ожидалось:
                    - Несостоявшиеся проекты и инициативы
                    -Отмененные события и решения 
                    -Люди, которые должны были встретиться, но не встретились
                    -Технологии, которые не были внедрены по неочевидным причинам
                    [ФОРМАТ ОТВЕТА]
                    Никаких вводных фраз. Не пиши "Вот список событий...", "Я проанализировал(а)..." или любые другие объяснения. Начинай ответ сразу с первого факта.
                    Все найденные факты разделяй пустой строкой.
                    В начале пишется название анализа: ИНВЕРСИВНЫЙ ПОИСК
                    [ПРИМЕР ВЫВОДA]
                    ИНВЕРСИВНЫЙ ПОИСК

                    ОПИСАНИЕ: [описание полученной информации]
        ...
        case 2:
            prompt = """
                    [ЗАДАЧА]
                    Проанализируй все PDF файлы и db_extension.txt.
                    Определи темы, которые систематически ИЗБЕГАЮТСЯ в источниках:
                    - О чем не говорят современники событий?
                    - Какие аспекты освещаются поверхностно?
                    - Где чувствуется самоцензура или внешнее давление?
                    - Какая информация могла быть намеренно уничтожена?
                    [ФОРМАТ ОТВЕТА]
                    Никаких вводных фраз. Не пиши "Вот список событий...", "Я проанализировал(а)..." или любые другие объяснения. Начинай ответ сразу с первого факта.
                    Все найденные факты разделяй пустой строкой.
                    В начале пишется название анализа: АНАЛИЗ УМОЛЧАНИЙ
                    [ПРИМЕР ВЫВОДА]
                    АНАЛИЗ УМОЛЧАНИЙ

                    ОПИСАНИЕ: [описание найденной темы]
        ...
        case 3:
            prompt = """
                    [ЗАДАЧА]
                    Проанализируй все PDF файлы и db_extension.txt.
                    Найди фигуры, которые НЕ на виду, но влияют на события:
                    - Люди из окружения, которые упоминаются вскользь
                    - Финансисты и спонсоры, остающиеся в тени
                    - Неформальные советники и консультанты
                    - Семейные и личные связи влиятельных людей
                    [ФОРМАТ ОТВЕТА]
                    Никаких вводных фраз. Не пиши "Вот список событий...", "Я проанализировал(а)..." или любые другие объяснения. Начинай ответ сразу с первого факта.
                    Все найденные факты разделяй пустой строкой.
                    В начале пишется название анализа: НЕВИДИМЫЕ ВЛИЯТЕТЕЛИ 
                    [ПРИМЕР ВЫВОДА]
                    НЕВИДИМЫЕ ВЛИЯТЕТЕЛИ

                    ОПИСАНИЕ: [описание найденной личности]
...
def get_stage3_prompt(lens_num):
    match lens_num:
        ...
        case 2:
            prompt = f"""
                    [ИНСТРУКЦИЯ]
                    Входные данные: Ты получаешь файлы [db_facts.txt, db_facts_checked_{lens_num-1}.txt], содержащие список гипотез и их проверку.
                    Задача:
                    Проанализируй каждую гипотезу.
                    Для каждой гипотезы найди все возможные альтернативные объяснения.
        ...
        case 3:
            prompt = f"""
                    [ИНСТРУКЦИЯ]
                    Входные данные: Ты получаешь файлы [db_facts.txt, db_facts_checked_{lens_num-2}.txt, db_facts_checked_{lens_num-1}.txt], содержащие список гипотез и их проверки.
                    Задача:
...
def get_stage4_prompt(num_series, blind_spots = False):
    if blind_spots:
        blind_spots_prompt = ["""
                        db_facts.txt: содержит информацию о слепых пятная в истории. 
                        """,
        "Если количества гипотез не хватает, возьми факты из db_facts.txt анализа слепых пятен."]
    else:
        blind_spots_prompt = ["", ""]
    ...
    [ВХОДНЫЕ ФАЙЛЫ]
    db_facts_checked.txt: Файлы с проверенными гипотезами (все сведения по одной гипотезе содержатся внутри блока [НАЧАЛО ПРОВЕРКИ ГИПОТЕЗЫ][КОНЕЦ ПРОВЕРКИ ГИПОТЕЗЫ]).
    {blind_spots_prompt[0]}
    Прикрепленные PDF-файлы: Содержат основной, детальный контент по теме.
    [КЛЮЧЕВЫЕ ЗАДАЧИ]
    Основа сценария (Скелет): Используй файлы db_facts_checked.txt как фундамент для ключевых тезисов и поворотных точек сценария.
    Фильтрация гипотез: Проверь основную гипотезу на: Нет подтверждения, Есть противоречия, Низкая вероятность или Есть противоречащие факты.
    Если найден негативный флаг, то отбрось гипотезу. Если такого флага нет, то возьми самую интригующую формулировку гипотезы (основную или альтернативную) и ее обоснование.
    {blind_spots_prompt[1]}
...
def get_stage5_prompt(ser:int, ch: int, previous_chapter_text: str):
    prompt = f"""
           [ИНСТРУКЦИЯ]
            ...
            db_facts_checked.txt: Файлы с проверенными гипотезами.
