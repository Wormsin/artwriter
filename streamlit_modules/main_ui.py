...
def show_main_app():
    ...
    with tab02:
        ...
        if projects_list:
            ...
            st.header("📂 Выберите Активный Проект")
            ...
            if active_project_id:
                #st.success(f"Активный проект: **{active_project_folder}** (ID: {active_project_id})")
                st.markdown("---")
                
                # --- ФОРМА РАСШАРИВАНИЯ ПРОЕКТА ---
                st.subheader("🤝 Расшарить доступ к проекту")
    ...
    if is_project_active:
        st.success(f"Активный проект: {st.session_state.active_project_folder}. Доступны следующие шаги.")
        ...
        with tab2:
            ...
            if st.button("Расширить БД", key='btn2'):
                try:
                    with st.spinner(f'Собираю данные для проекта "{st.session_state.active_project_folder}"...'):
                        result = expand_db(st.session_state.jwt_token, st.session_state.active_project_folder,st.session_state.active_project_id)
            ...
            if st.button("Найти Факты", key='btn3'):
                try:
                    with st.spinner(f'Ищу факты "{topic_folder}"...'):
                        result = search_facts(st.session_state.jwt_token, topic_folder, st.session_state.active_project_id)
                        # Обработка успешного ответа
            ...
            if st.button("Проверить Факты", key='btn4'):
                try:
                    with st.spinner(f'Проверяю факты "{topic_folder}"...'):
                        result = check_facts(st.session_state.jwt_token, topic_folder, st.session_state.active_project_id)
                        # Обработка успешного ответа
                ...
                try:
                    with st.spinner(f'Пишу сценарий "{topic_name}"...'):
                        result = write_scenario(st.session_state.jwt_token, st.session_state.active_project_folder, 
                                                st.session_state.active_project_id, max_tokens)
                        # Обработка успешного ответа
